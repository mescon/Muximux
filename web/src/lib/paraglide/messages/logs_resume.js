...
/** @typedef {{}} Logs_ResumeInputs */

const en_logs_resume = /** @type {(inputs: Logs_ResumeInputs) => LocalizedString} */ () => {
...
const ko_logs_resume = /** @type {(inputs: Logs_ResumeInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`재개`)
};
...
export const logs_resume = /** @type {((inputs?: Logs_ResumeInputs, options?: { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Logs_ResumeInputs, { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }, {}>} */ ((inputs = {}, options = {}) => {
	...
	if (locale === "uk") return uk_logs_resume(inputs)
	if (locale === "zh") return zh_logs_resume(inputs)
	if (locale === "es") return es_logs_resume(inputs)
	if (locale === "hi") return hi_logs_resume(inputs)
	if (locale === "pt") return pt_logs_resume(inputs)
	...
	if (locale === "ar") return ar_logs_resume(inputs)
	if (locale === "wuu") return wuu_logs_resume(inputs)
	if (locale === "mr") return mr_logs_resume(inputs)
	if (locale === "nb") return nb_logs_resume(inputs)
	if (locale === "fi") return fi_logs_resume(inputs)
	if (locale === "da") return da_logs_resume(inputs)
	if (locale === "et") return et_logs_resume(inputs)
	if (locale === "lv") return lv_logs_resume(inputs)
	if (locale === "lt") return lt_logs_resume(inputs)
	if (locale === "pl") return pl_logs_resume(inputs)
	if (locale === "de") return de_logs_resume(inputs)
	if (locale === "nl") return nl_logs_resume(inputs)
	if (locale === "fr") return fr_logs_resume(inputs)
	if (locale === "it") return it_logs_resume(inputs)
	if (locale === "hu") return hu_logs_resume(inputs)
	if (locale === "cs") return cs_logs_resume(inputs)
	if (locale === "ro") return ro_logs_resume(inputs)
	if (locale === "el") return el_logs_resume(inputs)
	if (locale === "bg") return bg_logs_resume(inputs)
	if (locale === "hr") return hr_logs_resume(inputs)
	if (locale === "sr") return sr_logs_resume(inputs)
	if (locale === "sk") return sk_logs_resume(inputs)
	if (locale === "sl") return sl_logs_resume(inputs)
	if (locale === "ko") return ko_logs_resume(inputs)
	return en_logs_resume(inputs)
});
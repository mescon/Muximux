...
export const nav_settings = /** @type {((inputs?: Nav_SettingsInputs, options?: { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Nav_SettingsInputs, { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }, {}>} */ ((inputs = {}, options = {}) => {
	...
	if (locale === "pt") return pt_nav_settings(inputs)
	...
	if (locale === "ar") return ar_nav_settings(inputs)
	if (locale === "wuu") return wuu_nav_settings(inputs)
	if (locale === "mr") return mr_nav_settings(inputs)
	if (locale === "nb") return nb_nav_settings(inputs)
	if (locale === "fi") return fi_nav_settings(inputs)
	if (locale === "da") return da_nav_settings(inputs)
	if (locale === "et") return et_nav_settings(inputs)
	if (locale === "lv") return lv_nav_settings(inputs)
	if (locale === "lt") return lt_nav_settings(inputs)
	if (locale === "pl") return pl_nav_settings(inputs)
	if (locale === "de") return de_nav_settings(inputs)
	if (locale === "nl") return nl_nav_settings(inputs)
	if (locale === "fr") return fr_nav_settings(inputs)
	if (locale === "it") return it_nav_settings(inputs)
	if (locale === "hu") return hu_nav_settings(inputs)
	if (locale === "cs") return cs_nav_settings(inputs)
	if (locale === "ro") return ro_nav_settings(inputs)
	if (locale === "el") return el_nav_settings(inputs)
	if (locale === "bg") return bg_nav_settings(inputs)
	if (locale === "hr") return hr_nav_settings(inputs)

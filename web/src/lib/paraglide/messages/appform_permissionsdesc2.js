...
/** @typedef {{}} Appform_Permissionsdesc2Inputs */

const en_appform_permissionsdesc2 = /** @type {(inputs: Appform_Permissionsdesc2Inputs) => LocalizedString} */ () => {
...
const ru_appform_permissionsdesc2 = /** @type {(inputs: Appform_Permissionsdesc2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Делегируйте возможности браузера встроенному фрейму. Без этого современные браузеры блокируют камеру, микрофон, геолокацию и подобные API для содержимого с другого домена.`)
};
...
const ko_appform_permissionsdesc2 = /** @type {(inputs: Appform_Permissionsdesc2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`내장된 iframe에 브라우저 기능을 위임합니다. 이 설정이 없으면 최신 브라우저는 교차 출처 콘텐츠에 대해 카메라, 마이크, 위치 정보 등의 API를 차단합니다.`)
};

/**
* | output |
* | --- |
* | "Delegate browser features to the embedded iframe. Without these, modern browsers block camera, microphone, geolocation, and similar APIs for cross-origin con..." |
*
* @param {Appform_Permissionsdesc2Inputs} inputs
...
*/
const appform_permissionsdesc2 = /** @type {((inputs?: Appform_Permissionsdesc2Inputs, options?: { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Appform_Permissionsdesc2Inputs, { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }, {}>} */ ((inputs = {}, options = {}) => {
	const locale = experimentalStaticLocale ?? options.locale ?? getLocale()
	if (locale === "en") return en_appform_permissionsdesc2(inputs)
	if (locale === "sv") return sv_appform_permissionsdesc2(inputs)
	if (locale === "uk") return uk_appform_permissionsdesc2(inputs)
	if (locale === "zh") return zh_appform_permissionsdesc2(inputs)
	if (locale === "es") return es_appform_permissionsdesc2(inputs)
	if (locale === "hi") return hi_appform_permissionsdesc2(inputs)
	if (locale === "pt") return pt_appform_permissionsdesc2(inputs)
	if (locale === "bn") return bn_appform_permissionsdesc2(inputs)
	if (locale === "ru") return ru_appform_permissionsdesc2(inputs)
	if (locale === "ja") return ja_appform_permissionsdesc2(inputs)
	if (locale === "vi") return vi_appform_permissionsdesc2(inputs)
	if (locale === "yue") return yue_appform_permissionsdesc2(inputs)
	if (locale === "tr") return tr_appform_permissionsdesc2(inputs)
	if (locale === "ar") return ar_appform_permissionsdesc2(inputs)
	if (locale === "wuu") return wuu_appform_permissionsdesc2(inputs)
	if (locale === "mr") return mr_appform_permissionsdesc2(inputs)
	if (locale === "nb") return nb_appform_permissionsdesc2(inputs)
	if (locale === "fi") return fi_appform_permissionsdesc2(inputs)
	if (locale === "da") return da_appform_permissionsdesc2(inputs)
	if (locale === "et") return et_appform_permissionsdesc2(inputs)
	if (locale === "lv") return lv_appform_permissionsdesc2(inputs)
	if (locale === "lt") return lt_appform_permissionsdesc2(inputs)
	if (locale === "pl") return pl_appform_permissionsdesc2(inputs)
	if (locale === "de") return de_appform_permissionsdesc2(inputs)
	if (locale === "nl") return nl_appform_permissionsdesc2(inputs)
	if (locale === "fr") return fr_appform_permissionsdesc2(inputs)
	if (locale === "it") return it_appform_permissionsdesc2(inputs)
	if (locale === "hu") return hu_appform_permissionsdesc2(inputs)
	if (locale === "cs") return cs_appform_permissionsdesc2(inputs)
	if (locale === "ro") return ro_appform_permissionsdesc2(inputs)
	if (locale === "el") return el_appform_permissionsdesc2(inputs)
	if (locale === "bg") return bg_appform_permissionsdesc2(inputs)
	if (locale === "hr") return hr_appform_permissionsdesc2(inputs)
	if (locale === "sr") return sr_appform_permissionsdesc2(inputs)
	if (locale === "sk") return sk_appform_permissionsdesc2(inputs)
	if (locale === "sl") return sl_appform_permissionsdesc2(inputs)
	return ko_appform_permissionsdesc2(inputs)
});
export { appform_permissionsdesc2 as "appForm_permissionsDesc" }
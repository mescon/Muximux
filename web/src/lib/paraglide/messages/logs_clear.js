...
const ko_logs_clear = /** @type {(inputs: Logs_ClearInputs) => LocalizedString} */ () => {
...
export const logs_clear = /** @type {((inputs?: Logs_ClearInputs, options?: { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Logs_ClearInputs, { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }, {}>} */ ((inputs = {}, options = {}) => {
	...
	if (locale === "uk") return uk_logs_clear(inputs)
	if (locale === "zh") return zh_logs_clear(inputs)
	if (locale === "es") return es_logs_clear(inputs)
	if (locale === "hi") return hi_logs_clear(inputs)
	...
	if (locale === "ar") return ar_logs_clear(inputs)
	if (locale === "wuu") return wuu_logs_clear(inputs)
	if (locale === "mr") return mr_logs_clear(inputs)
	if (locale === "nb") return nb_logs_clear(inputs)
	if (locale === "fi") return fi_logs_clear(inputs)
	if (locale === "da") return da_logs_clear(inputs)
	if (locale === "et") return et_logs_clear(inputs)
	if (locale === "lv") return lv_logs_clear(inputs)
	if (locale === "lt") return lt_logs_clear(inputs)
	...
	if (locale === "hu") return hu_logs_clear(inputs)
	if (locale === "cs") return cs_logs_clear(inputs)
	if (locale === "ro") return ro_logs_clear(inputs)
	if (locale === "el") return el_logs_clear(inputs)
	if (locale === "bg") return bg_logs_clear(inputs)
	if (locale === "hr") return hr_logs_clear(inputs)
	if (locale === "sr") return sr_logs_clear(inputs)
	if (locale === "sk") return sk_logs_clear(inputs)
	if (locale === "sl") return sl_logs_clear(inputs)
	if (locale === "ko") return ko_logs_clear(inputs)

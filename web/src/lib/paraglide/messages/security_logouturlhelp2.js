...
const sv_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Din autentiseringsleverantörs utloggningsändpunkt — rensar den externa sessionen vid utloggning`)
};

const uk_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Адреса виходу вашого провайдера автентифікації — очищає зовнішню сесію при виході`)
};

const zh_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`认证提供者的登出端点 — 登出时清除外部会话`)
};

const es_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Endpoint de cierre de sesión de tu proveedor de autenticación — cierra la sesión externa al salir`)
};

const hi_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`आपके प्रमाणीकरण प्रदाता का लॉगआउट एंडपॉइंट — साइन-आउट पर बाहरी सत्र समाप्त करता है`)
};

const pt_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Endpoint de encerramento de sessão do teu fornecedor de autenticação — encerra a sessão externa ao sair`)
};

const bn_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`আপনার প্রমাণীকরণ প্রদানকারীর লগআউট এন্ডপয়েন্ট — সাইন-আউটে বাহ্যিক সেশন সাফ করে`)
};

const ru_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Конечная точка выхода вашего провайдера аутентификации — очищает внешнюю сессию при выходе`)
};

const ja_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`認証プロバイダーのログアウトエンドポイント — サインアウト時に外部セッションをクリア`)
};

const vi_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Điểm cuối đăng xuất của nhà cung cấp xác thực — xóa phiên bên ngoài khi đăng xuất`)
};

const yue_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`認證供應商嘅登出端點 — 登出時清除外部工作階段`)
};

const tr_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Kimlik doğrulama sağlayıcınızın çıkış uç noktası — çıkışta harici oturumu sonlandırır`)
};

const ar_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`نقطة نهاية تسجيل الخروج لمزوّد المصادقة — تنهي الجلسة الخارجية عند تسجيل الخروج`)
};

const wuu_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`认证提供商的注销端点——注销时清除外部会话`)
};

const mr_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`तुमच्या ऑथ प्रदात्याचा लॉगआउट एंडपॉइंट — साइन आउट करताना बाह्य सत्र साफ करतो`)
};

const nb_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Autentiseringsleverandørens utloggingsendepunkt — tømmer den eksterne økten ved utlogging`)
};

const fi_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Todennuspalveluntarjoajan uloskirjautumispiste — tyhjentää ulkoisen istunnon uloskirjautuessa`)
};

const da_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Din godkendelsesudbyders logout-endpoint — rydder den eksterne session ved log ud`)
};

const et_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Teie autentimisteenuse väljalogimislõpp-punkt — kustutab välise sessiooni väljalogimisel`)
};

const lv_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Jūsu autentifikācijas nodrošinātāja izrakstīšanās galapunkts — izrakstīšanās laikā notīra ārējo sesiju`)
};

const lt_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Jūsų autentifikavimo teikėjo atsijungimo adresas — ištrina išorinę sesiją atsijungiant`)
};

const pl_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Endpoint wylogowania dostawcy uwierzytelniania — kończy sesję zewnętrzną przy wylogowaniu`)
};

const de_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Abmelde-Endpunkt Ihres Auth-Anbieters — beendet die externe Sitzung bei der Abmeldung`)
};

const nl_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Het uitlog-endpoint van je auth-provider — wist de externe sessie bij uitloggen`)
};

const fr_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Point de terminaison de déconnexion de ton fournisseur d'auth — efface la session externe lors de la déconnexion`)
};

const it_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Endpoint di disconnessione del tuo provider auth — cancella la sessione esterna alla disconnessione`)
};

const hu_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`A hitelesítési szolgáltató kijelentkezési végpontja — kijelentkezéskor törli a külső munkamenetet`)
};

const cs_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Endpoint odhlášení vašeho poskytovatele ověřování — ukončí externí relaci při odhlášení`)
};

const ro_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Punctul de deconectare al furnizorului de autentificare — șterge sesiunea externă la deconectare`)
};

const el_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Το endpoint αποσύνδεσης του παρόχου ελέγχου ταυτότητας — καθαρίζει την εξωτερική συνεδρία κατά την αποσύνδεση`)
};

const bg_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Крайната точка за изход на доставчика за удостоверяване — изчиства външната сесия при изход`)
};

const hr_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Krajnja točka za odjavu vašeg pružatelja autentifikacije — briše vanjsku sesiju pri odjavi`)
};

const sr_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Krajnja tačka za odjavu vašeg provajdera autentifikacije — briše spoljašnju sesiju pri odjavi`)
};

const sk_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Endpoint odhlásenia vášho poskytovateľa overovania — ukončí externú reláciu pri odhlásení`)
};

const sl_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Končna točka za odjavo vašega ponudnika avtentikacije — izbriše zunanjo sejo ob odjavi`)
};

const ko_security_logouturlhelp2 = /** @type {(inputs: Security_Logouturlhelp2Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`인증 공급자의 로그아웃 엔드포인트 — 로그아웃 시 외부 세션을 지웁니다`)
};
...
const security_logouturlhelp2 = /** @type {((inputs?: Security_Logouturlhelp2Inputs, options?: { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Security_Logouturlhelp2Inputs, { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }, {}>} */ ((inputs = {}, options = {}) => {
	...
	if (locale === "sl") return sl_security_logouturlhelp2(inputs)
	if (locale === "ko") return ko_security_logouturlhelp2(inputs)
	return en_security_logouturlhelp2(inputs)
});

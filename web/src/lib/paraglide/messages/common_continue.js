...
const ko_common_continue = /** @type {(inputs: Common_ContinueInputs) => LocalizedString} */ () => {
...
export const common_continue = /** @type {((inputs?: Common_ContinueInputs, options?: { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Common_ContinueInputs, { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }, {}>} */ ((inputs = {}, options = {}) => {
	...
	if (locale === "uk") return uk_common_continue(inputs)
	if (locale === "zh") return zh_common_continue(inputs)
	if (locale === "es") return es_common_continue(inputs)
	if (locale === "hi") return hi_common_continue(inputs)
	if (locale === "pt") return pt_common_continue(inputs)
	...
	if (locale === "ar") return ar_common_continue(inputs)
	if (locale === "wuu") return wuu_common_continue(inputs)
	if (locale === "mr") return mr_common_continue(inputs)
	if (locale === "nb") return nb_common_continue(inputs)
	if (locale === "fi") return fi_common_continue(inputs)
	if (locale === "da") return da_common_continue(inputs)
	if (locale === "et") return et_common_continue(inputs)
	if (locale === "lv") return lv_common_continue(inputs)
	if (locale === "lt") return lt_common_continue(inputs)
	if (locale === "pl") return pl_common_continue(inputs)
	if (locale === "de") return de_common_continue(inputs)
	if (locale === "nl") return nl_common_continue(inputs)
	if (locale === "fr") return fr_common_continue(inputs)
	if (locale === "it") return it_common_continue(inputs)
	if (locale === "hu") return hu_common_continue(inputs)
	if (locale === "cs") return cs_common_continue(inputs)
	if (locale === "ro") return ro_common_continue(inputs)
	if (locale === "el") return el_common_continue(inputs)
	if (locale === "bg") return bg_common_continue(inputs)
	if (locale === "hr") return hr_common_continue(inputs)
	if (locale === "sr") return sr_common_continue(inputs)
	if (locale === "sk") return sk_common_continue(inputs)
	if (locale === "sl") return sl_common_continue(inputs)
	if (locale === "ko") return ko_common_continue(inputs)

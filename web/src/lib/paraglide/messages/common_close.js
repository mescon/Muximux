...
export const common_close = /** @type {((inputs?: Common_CloseInputs, options?: { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Common_CloseInputs, { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }, {}>} */ ((inputs = {}, options = {}) => {
	...
	if (locale === "uk") return uk_common_close(inputs)
	if (locale === "zh") return zh_common_close(inputs)
	if (locale === "es") return es_common_close(inputs)
	if (locale === "hi") return hi_common_close(inputs)
	...
	if (locale === "wuu") return wuu_common_close(inputs)
	if (locale === "mr") return mr_common_close(inputs)
	if (locale === "nb") return nb_common_close(inputs)
	if (locale === "fi") return fi_common_close(inputs)
	if (locale === "da") return da_common_close(inputs)
	if (locale === "et") return et_common_close(inputs)
	if (locale === "lv") return lv_common_close(inputs)
	if (locale === "lt") return lt_common_close(inputs)
	if (locale === "pl") return pl_common_close(inputs)
	if (locale === "de") return de_common_close(inputs)
	if (locale === "nl") return nl_common_close(inputs)
	if (locale === "fr") return fr_common_close(inputs)
	if (locale === "it") return it_common_close(inputs)
	if (locale === "hu") return hu_common_close(inputs)
	if (locale === "cs") return cs_common_close(inputs)
	if (locale === "ro") return ro_common_close(inputs)
	if (locale === "el") return el_common_close(inputs)
	if (locale === "bg") return bg_common_close(inputs)
	if (locale === "hr") return hr_common_close(inputs)
	if (locale === "sr") return sr_common_close(inputs)
	if (locale === "sk") return sk_common_close(inputs)
	if (locale === "sl") return sl_common_close(inputs)
	return ko_common_close(inputs)
});
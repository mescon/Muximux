...
/** @typedef {{}} Settings_AddedInputs */

const en_settings_added = /** @type {(inputs: Settings_AddedInputs) => LocalizedString} */ () => {
...
const ko_settings_added = /** @type {(inputs: Settings_AddedInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`추가됨`)
};
...
export const settings_added = /** @type {((inputs?: Settings_AddedInputs, options?: { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Settings_AddedInputs, { locale?: "en" | "sv" | "uk" | "zh" | "es" | "hi" | "pt" | "bn" | "ru" | "ja" | "vi" | "yue" | "tr" | "ar" | "wuu" | "mr" | "nb" | "fi" | "da" | "et" | "lv" | "lt" | "pl" | "de" | "nl" | "fr" | "it" | "hu" | "cs" | "ro" | "el" | "bg" | "hr" | "sr" | "sk" | "sl" | "ko" }, {}>} */ ((inputs = {}, options = {}) => {
	...
	if (locale === "uk") return uk_settings_added(inputs)
	if (locale === "zh") return zh_settings_added(inputs)
	if (locale === "es") return es_settings_added(inputs)
	if (locale === "hi") return hi_settings_added(inputs)
	if (locale === "pt") return pt_settings_added(inputs)
	...
	if (locale === "ar") return ar_settings_added(inputs)
	if (locale === "wuu") return wuu_settings_added(inputs)
	if (locale === "mr") return mr_settings_added(inputs)
	if (locale === "nb") return nb_settings_added(inputs)
	if (locale === "fi") return fi_settings_added(inputs)
	if (locale === "da") return da_settings_added(inputs)
	if (locale === "et") return et_settings_added(inputs)
	if (locale === "lv") return lv_settings_added(inputs)
	if (locale === "lt") return lt_settings_added(inputs)
	if (locale === "pl") return pl_settings_added(inputs)
	if (locale === "de") return de_settings_added(inputs)
	if (locale === "nl") return nl_settings_added(inputs)
	if (locale === "fr") return fr_settings_added(inputs)
	if (locale === "it") return it_settings_added(inputs)
	if (locale === "hu") return hu_settings_added(inputs)
	if (locale === "cs") return cs_settings_added(inputs)
	if (locale === "ro") return ro_settings_added(inputs)
	if (locale === "el") return el_settings_added(inputs)
	if (locale === "bg") return bg_settings_added(inputs)
	if (locale === "hr") return hr_settings_added(inputs)
	if (locale === "sr") return sr_settings_added(inputs)
	if (locale === "sk") return sk_settings_added(inputs)
	if (locale === "sl") return sl_settings_added(inputs)
	if (locale === "ko") return ko_settings_added(inputs)
	return en_settings_added(inputs)
});
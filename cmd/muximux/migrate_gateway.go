...
// `server.gateway:` form), parses each `<domain> { reverse_proxy <upstream> }`
// block, and emits the equivalent YAML for the v3.1.0+
// `server.gateway_sites:` field on stdout.
//
// Anything we cannot represent in the structured form (custom matchers,
// multiple upstreams per host, third-party plugins, etc.) is reported on
// stderr with the originating host so the operator knows what to handle
// outside Muximux's gateway. The migrator is best-effort: it converts
// what it can and surfaces the rest, never silently drops directives.
//
// Usage:
//
...
// pure (no I/O, no os.Exit), so the test in migrate_gateway_test.go
// can drive it directly with table cases.
func migrateCaddyfileToSites(src []byte) ([]config.GatewaySite, []string, error) {
...
func routeToSite(route *caddyRoute, httpOnly bool) (*config.GatewaySite, []string) {
	...
	site := config.GatewaySite{Domain: host}
	if httpOnly {
		site.TLS = "none"
	}
...
func rpToSite(site *config.GatewaySite, h *caddyHandler, host string, warnings *[]string) {
...
func headersToSite(site *config.GatewaySite, h *caddyHandler, host string, warnings *[]string) {
	if h.Response != nil {
